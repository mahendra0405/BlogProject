...
namespace BlogLab.Repository
{
    public class BlogRepository : IBlogRepository
    {
        ...
        public async Task<List<Blog>> GetAllByUserIdAsync(int applicationUserId)
        {
            ...
            using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                blogs = await connection.QueryAsync<Blog>("Blog_GetUserById", new { ApplicationUserId = applicationUserId },
                    commandType: CommandType.StoredProcedure);
        ...
        public async Task<Blog> UpSertAsync(BlogCreate blogCreate, int applicationUserId)
        {
            ...
            {
                await connection.OpenAsync();
                newBlogId = await connection.ExecuteScalarAsync<int>("Blog_Insert",
